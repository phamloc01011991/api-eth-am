...
module.exports = function () {
  ...
  for (const key of Object.keys(this.config.logger).filter(key => isFunction(this.config.logger[key]))) {
    logger[key] = function () {
      ...
      if (typeof parameters[0] === 'string' && formatSpecifiers.some(t => parameters[0].includes(t)) && parameters[1]) {
        parameters[0] = format(parameters[0], parameters[1]);
        parameters[1] = undefined;
      }
      parameters[1] = isUndefined(parameters[1]) ? {} : _this.parseArg(parameters[1]);
      ...
      return _this.config.logger[key](...Array.prototype.slice.call(parameters));
    };
  }
  ...
  onFinished(res, err => {
    ...
    if (err) {
      logger[level](message || err.message, {
        err
      });
    } else if (message) {
